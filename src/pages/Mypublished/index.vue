<template> 
  <view>
    ...
    <view v-if="my_published.length == 0" class="text-gray text-bold text-center" id="msg">空空如也~</view>
    <view v-else>
    <view
      class="item-card cu-card article"
      v-for="(item, index) in my_published"
      :key="index"
    >
    ...
    remove(index, item) {
      ...
      db.collection("secondhand")
        ...
        .then(res => {
          ...
          setTimeout(function() {
            wx.hideLoading();
            that.my_published.splice(index, 1);
          }, 800);
        });
    }
  },
