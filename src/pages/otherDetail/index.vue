    ...
    <view class="item-card" v-for="(item, index) in secondhand" :key="index">
      <view>
        ...
        <view class="cu-form-group">
          ...
          <view class="cu-tag bg-green light xl" style="float: right; margin-top: 36rpx">发布者：{{item.nickname}}</view>
        </view>
        <view class="action">
          <view class="cu-form-group">
            <view class="title">内容类别：{{item.category}}</view>
          </view>
          <view class="cu-form-group">
      ...
      <button class="cu-btn bg-green shadow" v-if="userOpenId" @click="collect(item)">
        收藏
        <view class="cuIcon-favorfill"></view>
      </button>

      <button class="cu-btn bg-green shadow" v-if="userOpenId" @click="goToChat(item)">
        发起对话
